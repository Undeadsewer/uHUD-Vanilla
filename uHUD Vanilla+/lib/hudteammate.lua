Hooks:PostHook( HUDTeammate , "init" , "uHUDPostHUDTeammateInit" , function( self , i , teammates_panel , is_player , width )
	...
	if self._main_player then
		...
		local kill_counter = self._panel:text({
			...
			text 		= "  (0/0)",
			layer 		= 5,
			color 		= uHUD:HasSetting( "kill_counter_colour" ) and Color( uHUD:HasSetting( "kill_counter_colour" ) ) or Color.yellow,
			visible 	= uHUD:HasSetting( "kill_counter" ) or false,
			y 			= 0,
			vertical 	= "bottom",
			align 		= "left",
	...
	local voice = self._panel:bitmap({
		name 			= "voice",
...
Hooks:PostHook( HUDTeammate , "set_name" , "uHUDPostHUDTeammateSetName" , function( self , teammate_name )
	...
	if self._main_player and uHUD:HasSetting( "mask" ) or self._peer_id and uHUD:HasSetting( "mask_teammate" ) then
...
Hooks:PostHook( HUDTeammate , "set_callsign" , "uHUDPostHUDTeammateSetCallsign" , function( self , id )
	...
	self._panel:child( "voice" ):set_color( tweak_data.chat_colors[ id ] )
	...
	if not self._main_player and self:peer_id() and managers.network:session() and managers.network:session():peer( self:peer_id() ):is_cheater() then
		...
		self._panel:child( "infamy_icon" ):set_color( tweak_data.screen_colors.pro_color )
		self._panel:child( "infamy_rank" ):set_color( tweak_data.screen_colors.pro_color )
		self._panel:child( "voice" ):set_color( tweak_data.screen_colors.pro_color )
	end
...
Hooks:PreHook( HUDTeammate , "teammate_progress" , "uHUDPreHUDTeammateTeammateProgress" , function( self , enabled , tweak_data_id , timer , success )
	...
	if enabled and not self._main_player and self:peer_id() and uHUD:HasSetting( "interaction_text_teamamte" ) then
...
Hooks:PostHook( HUDTeammate , "set_ammo_amount_by_type" , "uHUDPostHUDTeammateSetAmmoAmountByType" , function( self , type , max_clip , current_clip , current_left , max )
	...
	color_total = color_total or max_ammo and ( uHUD:HasSetting( "full_ammo_colour" ) and Color( uHUD:HasSetting( "full_ammo_colour" ) ) or ( Color( 255 , 41 , 204 , 122 ) / 255 ) )
	...
	if uHUD:HasSetting( "animated_total_ammo" ) and self._last_ammo and self._last_ammo[ type ] and self._last_ammo[ type ] < current_left then
	...
	if uHUD:HasSetting( "animated_current_mag" ) and self._last_clip and self._last_clip[ type ] and self._last_clip[ type ] < current_clip then
...
function HUDTeammate:set_stamina_value( value )
	...
	radial_stamina:set_visible( uHUD:HasSetting( "stamina" ) and true or false )
...
function HUDTeammate:set_infinite_ammo( activated , ammo_clip )
...
function HUDTeammate:update_kill_counter( weapon , total )
	...
	self._panel:child( "kill_counter" ):set_visible( uHUD:HasSetting( "kill_counter" ) and true or false )
...
function HUDTeammate:set_voice( active )

	self._panel:child( "voice" ):set_visible( active )
	
	self._panel:child( "infamy_rank" ):set_visible( not active and uHUD:HasSetting( "infamy_rank" ) )
	self._panel:child( "infamy_icon" ):set_visible( not active and uHUD:HasSetting( "infamy_callsign" ) )
	self._panel:child( "callsign" ):set_visible( not active and not uHUD:HasSetting( "infamy_rank" ) and not uHUD:HasSetting( "infamy_callsign" ) )
...
function HUDTeammate:_animate_callsign( callsign )
	...
	while true do
		...
		t = t + coroutine.yield()
		callsign:set_color( Color( uHUD:ConvertToRGB( math.sin( ( 7 * t ) % 80 ) , 1 , 1 ) ) )
