function DoctorBagBase:_create_workspace()

	self._gui = World:newgui()
	self._ws = self._gui:create_world_workspace( 100 , 100 , self._unit:position() + Vector3( -25 , 0 , 50 ) , Vector3( 50 , 0 , 0 ) , Vector3( 0 , 0 , -50 ) )
	self._ws:set_billboard( self._ws.BILLBOARD_BOTH )
	
	self._ws_t = self._ws:panel():text({
		name 		= "amount",
		text 		= tostring( self._amount or 0 ),
		y 			= 0,
...
function DoctorBagBase:_destroy_workspace()
...
function DoctorBagBase:_attach()
...
Hooks:PostHook( DoctorBagBase , "_set_visual_stage" , "uHUDPostDoctorBagBaseSetVisualStage" , function( self )
...
Hooks:PreHook( DoctorBagBase , "update" , "uHUDPreDoctorBagBaseUpdate" , function( self , unit , t , dt )

	if self._empty then self:_destroy_workspace() return end
	if not self._ws then self:_create_workspace() end
	
	self._ws:set_world( 100 , 100 , self._unit:position() + Vector3( -25 , 0 , 50 ) , Vector3( 50 , 0 , 0 ) , Vector3( 0 , 0 , -50 ) )
	self._ws_t:set_text( tostring( self._amount or 0 ) )

end )

Hooks:PostHook( DoctorBagBase , "destroy" , "uHUDPostDoctorBagBaseDestroy" , function( self )
